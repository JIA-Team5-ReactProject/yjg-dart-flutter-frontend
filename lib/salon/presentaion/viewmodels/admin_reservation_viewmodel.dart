...
final reservationViewModelProvider =
    StateNotifierProvider<ReservationViewModel, AsyncValue<List<Reservation>>>(
        (ref) {
  return ReservationViewModel(ref.read(fetchReservationsUseCaseProvider));
});

final fetchReservationsUseCaseProvider =
    Provider((ref) => FetchReservationsUseCase(AdminReservationDataSource()));

class ReservationViewModel
    extends StateNotifier<AsyncValue<List<Reservation>>> {
  final FetchReservationsUseCase _fetchReservationsUseCase;
  String selectedDate = '';
  int todayReservationCount = 0; // 현재 날짜 예약 개수

  ReservationViewModel(this._fetchReservationsUseCase)
  ...
  Future<void> fetchReservations({required String reservationDate}) async {
    ...
    try {
      ...
      todayReservationCount = reservations.length; // 현재 날짜 예약 개수 업데이트
