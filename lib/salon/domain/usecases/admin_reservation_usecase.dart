...
class FetchReservationsUseCase {
  ...
  Future<List<Reservation>> call(String reservationDate) async {
    ...
    return reservationsJson.map<Reservation>((json) => Reservation.fromJson(json)).toList();
  }
  ...
  Future<ReservationResult> updateStatus(AdminReservation reservation) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return ReservationResult(
            isSuccess: false, message: '미용실 예약 상태 업데이트 실패: ${response.body}');
      }
    } catch (e) {
      return ReservationResult(isSuccess: false, message: '미용실 예약 상태 변경 중 오류 발생 : $e');
    }
  }
}
